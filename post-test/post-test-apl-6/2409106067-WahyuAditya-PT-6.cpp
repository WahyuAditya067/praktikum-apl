...
Pembalap pembalap[maxpembalap] = {
    ...
    {"Yuki Tsunoda", "Red Bull Racing", "Jepang", "22", "11 Mei 2000", 0},
    ...
    {"Liam Lawson", "Racing Bulls", "Selandia Baru", "30", "11 Februari 2002", 0},
    ...
    {"Nico Hulkenberg", "Sauber", "Jerman", "27", "19 Agustus 1987", 0},
...
void jumlahpodium() {
    for (int i = 1; i < jumlahpembalap; i++) {
        Pembalap key = pembalap[i];
        int j = i - 1;

        while (j >= 0 && pembalap[j].jumlahPodium < key.jumlahPodium) {
            pembalap[j + 1] = pembalap[j];
            j--;
        }
        pembalap[j + 1] = key;
    }
    cout << "\nJumlah Podium Pembalap :" << endl;
    for (int i = 0; i < jumlahpembalap; i++) {
        cout << i+1 << ". " << pembalap[i].nama << " (" << pembalap[i].jumlahPodium << " podium)" << endl;
    }
}

void tampilkanpembalap() {
    for (int i = 0; i < jumlahpembalap - 1; i++) { 
        for (int j = 0; j < jumlahpembalap - i - 1; j++) { 
            if (pembalap[j].nama > pembalap[j + 1].nama) {
                Pembalap temp = pembalap[j];
    ...
    cout << "\nDaftar Pembalap F1 2025:" << endl;
    for (int i = 0; i < jumlahpembalap; i++) { 
        cout << i + 1 << ". " << pembalap[i].nama << endl;
    }
}
...
void tampilkantim() {
    for (int i = 0; i < jumlahtim - 1; i++) { 
        ...
        for (int j = i + 1; j < jumlahtim; j++) { 
            if (tim[j].nama < tim[indeksMin].nama) {
                indeksMin = j;
            }
        }
        if (indeksMin != i) {
    ...
    cout << "Daftar Tim F1 2025:\n";
    for (int i = 0; i < jumlahtim; i++) { 
        cout << i + 1 << ". " << tim[i].nama << endl;
    }
}

bool registrasiuser(User* users, int* jumlahuser, int maxuser) {
...
string gantipembalap(Pembalap pembalap[], int& jumlahpembalap) {
    int gantipembalap;
    ...
    if (gantipembalap >= 1 && gantipembalap <= jumlahpembalap) {
        ...
        string namaLama = pembalap[index].nama;
        cin.ignore();
        cout << "\nMasukkan data pembalap baru:\n";
        ...
        cout << "Jumlah Podium: "; cin >> pembalap[index].jumlahPodium;
        cout << "Pembalap " << namaLama << " berhasil diganti dengan " << pembalap[index].nama << "\n";
        return namaLama;
...
int main() {
    ...
    if (pilihanmenu == "1") {
        (registrasiuser(users, &jumlahuser, maxuser));
        enter();
    }
        ...
        tampilkanpembalap();
        gantipembalap(pembalap, jumlahpembalap);
        enter();
        }
        ...
        cout << "3. Lihat jumlah podium pembalap\n";
